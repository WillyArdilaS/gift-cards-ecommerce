...
const CardItem = ({ info }) => {
    ...
    return (
        <article id={info.id_tarjeta} className="mt-12 mx-auto">
            <h2 className="text-lg text-center text-slate-200 font-semibold"> {info.nombre} </h2>
            <h3 className="mt-2 text-lg text-center text-slate-200"> ${info.precio} </h3>
            <img src={info.ruta_imagen} alt="Imagen de la tarjeta" id="giftCardImage" className="w-56 h-36 mt-4 rounded-2xl object-cover" />

            <button type="button" className="text-white bg-[#050708] font-medium rounded-lg text-sm px-5 py-2.5 mt-6 mr-6 ml-6 
            text-center inline-flex items-center dark:focus:ring-[#050708]/50 dark:hover:bg-[#050708]/30" onClick={onAdd}>
